...
func _physics_process(delta: float) -> void:
	if fire_enabled:
		...
		if tick <= 0:
			...
			# shoot
			var b = preload("TankBullet.tscn").instance()
	...
	if abs(rot - dst_rot) > 0.1:
		rot = Global.step(rot, dst_rot, delta * SPEED * 4)
		rotation_degrees = rot
		speed *= 0.75
	elif front.is_colliding(): dst_rot -= 90 * dir
	elif not down.is_colliding(): dst_rot += 90 * dir

	move_and_slide(Vector2(dir * speed, speed * 10).rotated(rotation) +
				   Vector2(0, Global.world.SPEED))

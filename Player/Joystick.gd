...
static func clamp_vector2(v: Vector2, minv: Vector2, maxv: Vector2) -> Vector2:
	return Vector2(clamp(v.x, minv.x, maxv.x),
...
func update_input_event(event: InputEvent) -> void:
	if not event:
		...
		$MoveRect.rect_position = event.position - $MoveRect.rect_size / 2
		$MoveRect.visible = true

	input_event = event
	if input_event:
		input_event.position = clamp_vector2(input_event.position,
											 $MoveRect.rect_position,
											 $MoveRect.rect_position + $MoveRect.rect_size)

		var p = input_event.position - $MoveRect.rect_position
		p = clamp_vector2(p, Vector2.ZERO, $MoveRect.rect_size)
		update_dx(inverse_lerp($MoveRect.rect_size.x * 0.5, $MoveRect.rect_size.x, p.x))
		update_dy(inverse_lerp($MoveRect.rect_size.y * 0.5, $MoveRect.rect_size.y, p.y))
	else:
		update_dx(0)
		update_dy(0)
